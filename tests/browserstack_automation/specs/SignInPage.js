...
beforeEach(async () => {
  ...
  await SignIn.signInLinkBttn.click();
});
...
describe('SignIn', () => {
  ...
  /* it('verifyColorForContents', async () => {
    ...
    // Updated colors (WV-854), also commenting this test script as suggested since the design might change in future.
  ...
  it('verifyAppleSignInLinkResponsiveness', async () => {
    ...
    await driver.waitUntil(async () => await driver.getTitle() === 'Sign in to Apple Account', {
      timeout: 5000,
    });
    await driver.back();
  ...
  it('verifyTabKeyFunctionality', async () => {
    ...
    await driver.keys('Tab', 'Tab', 'Tab', 'Tab', 'Tab', 'Tab');
    await expect(await SignIn.emailFieldElement).toBeFocused();
  });
  ...
  it('verifyEmailFieldPasteFunctionalityUsingKeyboard', async () => {
    ...
    const { platformName } = browser.capabilities; // Gets the platform like 'Windows', 'macOS'
  ...
  it('verifyEmailFieldAcceptsOnlyLatinLetters', async () => {
    ...
    const isDisplayed = await  SignIn.sendVerificationEmailBttnElement.isClickable();
  ...
  it('verifyEmailFieldAcceptsLettersWithNum', async () => {
    ...
    const isDisplayed = await (await SignIn.sendVerificationEmailBttnElement).isClickable();
    await expect(isDisplayed).toBe(true);
  });
  // SignIn_034
  ...
  it('verifyEmailFieldDoesntAcceptStartWithDot', async () => {
    ...
    const isDisplayed = await (await SignIn.sendVerificationEmailBttnElement).isEnabled();
  ...
  it('verifyBackButtonOnVerificationPage', async () => {
    ...
    await driver.waitUntil(async () => await (await SignIn.codeVerifyElement).getText() === 'Code Verification', {
      timeout: 5000,
    });
